...
{
    public class ChangeColourView : ComputerView
    {
        ...
        public override void OnShow(object[] args)
        {
            ...
            if (Plugin.Instance.TheChanger == "Hat")
            {
                _color = Plugin.Instance.HatColour.Value;
                SetColor(_color);
            }
            if (Plugin.Instance.TheChanger == "Badge")
            {
                _color = Plugin.Instance.BadgeColour.Value;
                SetColor(_color);
            }
            if (Plugin.Instance.TheChanger == "Hold")
            {
                _color = Plugin.Instance.HoldableColour.Value;
                SetColor(_color);
            }
            if (Plugin.Instance.TheChanger == "Chest")
            {
                _color = Plugin.Instance.ChestColour.Value;
                SetColor(_color);
            };
            _savedColor = _color;
            Redraw();
        ...
        void _SetColour(Color col) 
        { 
            if (Plugin.Instance.TheChanger == "Hat")
            { 
                Plugin.Instance.HatColour.Value = col;
                Plugin.Instance.UpdateProps();
                ShowView<Settings>();
            } 
            if (Plugin.Instance.TheChanger == "Badge")
            { 
                Plugin.Instance.BadgeColour.Value = col;
                Plugin.Instance.UpdateProps();
                ShowView<Settings>();
            } 
            if (Plugin.Instance.TheChanger == "Hold") 
            { 
                Plugin.Instance.HoldableColour.Value = col;
                Plugin.Instance.UpdateProps();
                ShowView<Settings>();
            } 
            if (Plugin.Instance.TheChanger == "Chest") 
            { 
                Plugin.Instance.ChestColour.Value = col;
                Plugin.Instance.UpdateProps();
                ShowView<Settings>();
            } 
        }
